...
client = wolframalpha.Client('RHY8T9-5W4YRXAX3T')
...
def talk(audio):
    print('Personal Assisstant: ' + audio)
    engine.say(audio)
    engine.runAndWait()
...
talk('Hey Chirag, It\'s  your assistant !')
        ...
        if 'open google' in Input:
            ...
            if 'me' in recipient:
                try:
                    ...
                    server.sendmail('chiragnahata05@gmail.com', "Recipient_Username", content)
                    server.close()
                    talk('Email sent!')
